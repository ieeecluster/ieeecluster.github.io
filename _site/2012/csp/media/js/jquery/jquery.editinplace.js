...
$.extend(InlineEditor.prototype, {
	...
	replaceContentWithEditor: function() {
		...
		this.dom.html("");
		this.dom.append('<form class="inplace_form" style="display: inline; margin: 0; padding: 0;"></form>');
		this.dom.find('form').append(editorElement);
	...
	createEditorElement: function() {
		...
		if ("select" === this.settings.field_type)
			...
			editor = $('<input type="text" ' + this.inputNameAndClass() 
				+ ' size="' + this.settings.text_size  + '" maxlength=100/>');
		else if ("textarea" === this.settings.field_type)
			editor = $('<textarea ' + this.inputNameAndClass() 
				+ ' rows="' + this.settings.textarea_rows + '" '
				+ ' cols="' + this.settings.textarea_cols + '" maxlength=200/>');
	...
	setInitialValue: function() {
		...
		var editor = this.dom.find(':input:first');
		...
		if(initialValue=='(Click here to add text)'){
			editor.val('');
		}
	},
	...
	handleCancelEditor: function(anEvent) {
		...
		/*xzj 2012-7-19 删除以下代码   this.restoreOriginalValue();保存回原来的值，下段代码又将修改后的值覆盖掉上句话存入的原有的值
