...
(function($) { // create private scope (inside you can use $ instead of jQuery)
		...
		function newDatepickerHTML () {
			...
			table.append('<tbody></tbody>');
				...
				for(var ii=0;ii<12;ii++)
					monthselect += '<option value="'+ii+'">'+months[ii]+'</option>';
				monthselect += '</select>';
				// year select field
			...
			if ( jQuery(this).is('input') && 'text' == jQuery(this).attr('type')) {
				...
				jQuery(this).click(function (ev) {
					...
					if (false == jQuery.data($this.get(0), "simpleDatepicker").hasDatepicker) {
						...
						//add by yihua zheng for yyyy-MM-dd format
						var initialDate = $this.val().replace(/^(\d{4})-(\d{1,2})-(\d{1,2})$/,"$2/$3/$1");
						...
						if (opts.parentEl!=null){
							jQuery(opts.parentEl).click(function(e){if (e.target!=ev.target){closeIt($this,datepicker)}});
						}
						...
						opts.beforeShow($this.get(0), datepicker);
					}
	...
	jQuery.fn.simpleDatepicker.formatOutput = function (dateObj) {
		...
		//change by yihua zheng format: yyyy-MM-dd
