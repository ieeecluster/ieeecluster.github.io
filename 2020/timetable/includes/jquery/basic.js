...
function count_words_jq(textfield_name, countfield_id) {
    ...
    $("#" + countfield_id).html(count_words_in_str(val));
}

function tiny_mce_count_words_jq(tinymce_id, countfield_id) {
    var field = tinymce.get(tinymce_id);
    if (field != undefined) {
        val = field.getContent();
        $("#" + countfield_id).html(count_words_in_str(val));
    }
}

// This isn't treating html tags exactly like our python word count is.
function count_words_in_str(val) {
    val = strip_html(val);
    // clean space at the beginning and end of lines
    // val = val.replace(/^\s+/g, "").replace(/\s+$/g, "");
    val = val.trim();
    // This is strange but it fixes the empty string case.
    if (val) { val = val + ' '; }
    return val.split(/\s+/).length - 1;
}
...
function replaceElWithGetResponseOverlay($el, get_url, loading_msg, callback) {
  add_message_overlay($el, loading_msg);
  $.ajax({
    url: get_url,
    type: 'GET',
...
function _setup_info_links() {
    ...
    var qtip_content_helper = function(el, no_clone) {
        ...
        if ($($(el).attr('title')).length > 0) {
            title_opts = {
                ...
                text: function(api) { var title_id = $(this).attr('oldtitle'); return title_id ? $(title_id).clone() : null; },
                button: true
            }
        } else {
            title_opts = ''
    ...
    // Try to put it within the form so that buttons work.
    if ($('form#thisForm').length > 0) {
      var position_args = {adjust: {method: 'shift shift'}, container: $('form#thisForm'), viewport: $(window)};
    } else {
      var position_args = {adjust: {method: 'shift shift'}, viewport: $(window)};
    }
    ...
    $('.ttip_object_info, .ttip_object_info_blue, .ttip_object_info_white').each(function() {
        // pull other attributes out of the object
        var width_class = $(this).attr('width_class')

        var no_clone = false;
        if ($(this).attr('clone') == 'no_clone') {
            no_clone = true;
        }
        ...
        var classes = 'qtip-rm qtip-rounded rm-qtip '
        var position = position_args;
        if ($(this).attr('modal') == 'modal') {
            show['event'] = 'click';
            show['modal'] = { on: true };
            hide = { event: 'unfocus', leave: false };
            position = modal_position;
            classes = 'qtip-rm qtip-rounded rm-qtip rm-qtip-modal '
        }

        $(this).qtip({
            content: qtip_content_helper(this, no_clone=no_clone),
            show: show,
            hide: hide,
            style: {widget: false, classes: classes + width_class, tip: {width: 15, height: 10}},
            position: position
        })
    })
}
...
function _______MARKDOWN______() { }
...
var escape_parens_for_mathjax = function (converter) {
    // it could be that we just want to automatically replace slashes with double slashes
    return [
        {
            type: 'lang',
            regex: /\\\(.*?\\\)/g,
        ...
        },
        {
            ...
            regex: /\\\[.*?\\\]/g,
            replace: function (match, content) {
                // This one seems to work, but will probably break if [] is used inside the latex
...
var escape_greater_thans_for_blockquote = function (converter) {
    // Allow greater thans as the first thing on a line.
    return [
        {
            type: 'lang',
            regex: /<br>\s*&gt;/gm,
...
function __markdown(text) {
    var converter = new showdown.Converter({
        ...
        extensions: [escape_parens_for_mathjax, escape_greater_thans_for_blockquote],
...
function emulate_prep_text(text) {
    // Treat text the same way we do coming out of the db. See html_util.prep_text()
    text = text.replace(/&/g, '&amp;');
...
function unescape_for_showdown_display(text) {

    // Showdown double-escapes '<>' in code sections. Undo this for display.
    text = text.replace(/&amp;/g, '&');
...
function preview_markdown(source_id, target_id, render_mathjax) {
    var text = document.getElementById(source_id).value;

    text = emulate_prep_text(text);

    var target = document.getElementById(target_id);
    var html = __markdown(text);

    html = unescape_for_showdown_display(html)

    if (!html) {
        html = "<br><br><div style='color: gray; font-style: italic; text-align: center;'>No text has been entered yet, so there is nothing to preview.</div><br><br>"
    }
...
// This allows common punctuation as well as spaces for the character after a closing *.
var good_markdown_star_regex = /(?:\s|^)\*+[^\*]+\*+(?:\s|$|[.,:;])/gm
function looks_like_math(paragraph) {
    var matches = paragraph.match(/\*+/g);
    if (matches && matches.length > 1) {
        // Remove cases that look like good italics.
        paragraph = paragraph.replace(good_markdown_star_regex, '')
        // Any remaining *s look like math.
        var math_star_matches = paragraph.match(/\*+/g);
        if (math_star_matches != null) {
            return true;
        }
    }
...
var markdown_sects = 0;
function process_markdown(i, el) {
    var contents = $(el).clone();

    contents.find("br").replaceWith("\n");
    var result = __markdown(contents.html());

    // replace <p> with <br><br>
    var comp_result = $('<div>' + result + '</div>');
    comp_result.find('p').replaceWith(function(el) {
        var ans = $(this).html();
        if (comp_result.find('p').length > 1) {
            ans = ans + '<br><br>';
        }
        return ans;
    })

    result = comp_result.html()
    // Get rid of trailing BRs
    result = result.replace(/(<br>\s*)+$/, '')
    // Get rid of extra spaces
    // result = result.replace(/\s+/g, ' ').trim()

    if (result.trim().replace(/\s+/g, '').replace(/<br>/g, '') == contents.html().trim().replace(/\s+/g, '')) {
        return;
    } else {
        //  If a paragraph looks like it contains *s in a mathy way don't show the markdown view on load.
        var show_orig = false;
        var paragraphs = contents.html().split('\n\n');
        $(paragraphs).each(function(i, el) {
            show_orig = looks_like_math(this);
            if (show_orig) { return false; };
        });

        var orig_markdown_sect_class = 'markdown_sect_' + markdown_sects;
        var new_markdown_sect_class = orig_markdown_sect_class + '_markdown';
        var toggle_link_class = orig_markdown_sect_class + '_toggle';

        markdown_sects = markdown_sects + 1;

        // Add class to old section
        $(el).wrapInner('<div class="' + orig_markdown_sect_class + '">');
        $('.' + orig_markdown_sect_class).hide();

        result = unescape_for_showdown_display(result)

        // Put in new section
        // var new_sect = $("<div style='color: red;'>" + result + "</div>");
        var new_sect = $("<div>" + result + "</div>");
        new_sect.addClass(new_markdown_sect_class);
        $('.' + orig_markdown_sect_class).after(new_sect);

        // Put in toggle links
        var links = $(`
            <span class="` + toggle_link_class + `">
                <div class="active_toggle_button clickable" title="Toggle whether the text is formatted by Markdown or not.">markdown</div>
            </span>
        `);
        links.push($(`
            <span class="` + toggle_link_class + `">
                <div class="inactive_toggle_button clickable" title="Toggle whether the text is formatted by Markdown or not.">markdown</div>
            </span>
        `).hide());

        var wrapper = $('<div style="float: right;"></div>');
        links.insertBefore('.' + orig_markdown_sect_class);
        links.wrap(wrapper);

        var clear = $('<div></div>').addClass('clear');
        clear.insertBefore('.' + orig_markdown_sect_class);

        $('.' + toggle_link_class).click(function () {
            $('[class^=' + orig_markdown_sect_class).toggle();
        })
...
function toggle_monospace(el) {
    $(el).toggleClass('monospace');
    $(el).find('.toggle_monospace').toggle();
}


function process_monospace(i, el) {

    // Put in toggle links
    var links = $();
    links.push($(`
        <span class="toggle_monospace">
            <div class="inactive_toggle_button clickable" title="Toggle whether the font letter-width is fixed or proportional.">
                monospace
            </div>
        </span>
    `).hide());
    links.push($(`
        <span class="toggle_monospace">
            <div class="active_toggle_button clickable" title="Toggle whether the font letter-width is fixed or proportional.">
                monospace
            </div>
        </span>
    `));
    ...
    var wrapper = $('<div style="float: right;"></div>');
    links.prependTo(wrapper);
    wrapper.prependTo(el);

    // This may already be there from the markdown
    if ($(el).find('.clear').length == 0){
        var clear = $('<div></div>').addClass('clear');
        clear.insertAfter(wrapper);
    }

    $(el).find('.toggle_monospace').click(function() {toggle_monospace(el);})
    toggle_monospace(el);
}
...
// Process any elements that should be displayed using a markdown interpreter
$(function () {
    $('.markdown_display').each(process_markdown);
    $('.monospace').each(process_monospace);
})
...
function contains_html(field_sel) {
    // Returns true if any fields selected by <field_sel> contains any HTML tags.
    // note that this detects "<ANYTHING>" as html, but not "< div>" which is standard browser behavior

    var found = false;

    $(field_sel).each(function() {
        // I don't know why there's a try here
        try {
            var contents = $(this).val() // this is a string
            contents = contents.trim(); // Get rid of starting / trailing white space

            if (contents.length == 0) {
              // No contents - skip to the next field
              return true;
            }

            var parsed = $.parseHTML(contents);  // Returns an array of contents elements
            if (parsed.length > 1) {
              // multiple items means that there is more than just text.
              // If it was just text it would have a single #text element
              found = true;
            } else if ($(parsed[0]).nodeName.toLowerCase() != '#text') {
              // if the parsed text isn't #text then it is wrapped in a tag.
              found = true;
            }
